...
def run_demo():
    ...
    rospy.sleep(1)
    x1, y1 , theta1 = robot.get_robot_pose()
    distance = float(robot.getDistanceToFlag())
    goal_x = math.cos(theta1) * distance
    goal_y = math.sin(theta1) * distance
    Kpv = 0.9
    Kpa = 2
    while not rospy.is_shutdown():
        ...
        if distance>46:
            ...
            print(f"{robot_name} desired_angle = ", desired_angle_goal)

            #Here -0.05 to adjust the right position of the goal, 
            #since we can't get directly the goal coordinates, 
            #we should calculate from the distance information,
            #and there's a little error between these two values.
            angle = (desired_angle_goal-theta)*Kpa
